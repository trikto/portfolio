...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="site concept-cloud blog-page article-page"><div className="utility"><span className="pulse" /> DEVOPS FIELD NOTES</div><SiteHeader /><article className="article-shell"><header className="article-header"><Link className="article-back" href="/blog">← Back to articles</Link><div className="article-meta"><time dateTime={post.publishedAt}>{displayDate(post.publishedAt)}</time><span>{post.readingMinutes} min read</span>{post.updatedAt && post.updatedAt !== post.publishedAt ? <span>Updated {displayDate(post.updatedAt)}</span> : null}</div><h1>{post.title}</h1><p>{post.description}</p>{post.tags.length ? <div className="article-tags" aria-label="Article topics">{post.tags.map((tag) => <span key={tag}>{tag}</span>)}</div> : null}{post.coverImage ? <div className="article-hero-image"><Image alt={`${post.title} cover`} fill priority sizes="(max-width: 900px) 100vw, 900px" src={post.coverImage} /></div> : null}</header><div className="article-prose">{content}</div><footer className="article-footer"><div><span>Written by</span><strong>{post.author}</strong></div>{post.mediumUrl ? <a href={post.mediumUrl} target="_blank" rel="noopener noreferrer">Also available on Medium ↗</a> : null}</footer></article>{related.length ? <section className="article-related" aria-labelledby="related-title"><p className="eyebrow">KEEP READING</p><h2 id="related-title">Related articles</h2><ArticlesGrid articles={related} /></section> : null}<SiteFooter /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replaceAll("<", "\\u003c") }} /></main>;
}
