...
import { createStoredFile, encryptFile, exceedsFileLimit, fileShareLink, formatFileBytes, MAX_FILE_BYTES, MAX_STORED_PAYLOAD_BYTES, type FileShareFailure } from "@/lib/file-share";
...
export function FileComposer() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const result = await createStoredFile(payload);
  ...
  return <form className="secret-composer" onSubmit={submit}>
    <section className="secret-panel">
      ...
      <div className="secret-controls">
        <button className="button primary" type="submit" disabled={busy}>{busy ? "Encrypting..." : "Create share link"}</button>
      </div>
      {error && <p className="secret-error" id="file-error" role="alert">{error}</p>}
      <p className="secret-note">Encryption happens in this browser. Only ciphertext is uploaded, and the key stays in the link fragment.</p>
    </section>
  </form>;
}
