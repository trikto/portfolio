...
function fieldsFrom(expression: string): CronFields { return [...expression.trim().split(/\s+/).filter(Boolean), "", "", "", "", ""].slice(0, 5) as CronFields; }

export function CronEditor() {
  const [expression, setExpression] = useState(cronExpression(initial)), [timeZone, setTimeZone] = useState("UTC"), [copyState, setCopyState] = useState("");
  const [preview, setPreview] = useState({ expression: cronExpression(initial), timeZone: "UTC" });
  ...
  const fields = useMemo(() => fieldsFrom(expression), [expression]);
  const errors = useMemo(() => expression.trim().split(/\s+/).filter(Boolean).length === 5 ? validateCron(fields) : [{ field: -1, message: "Use exactly five space-separated fields." }], [expression, fields]);
  useEffect(() => { const id = window.setTimeout(() => setPreview({ expression, timeZone }), 250); return () => window.clearTimeout(id); }, [expression, timeZone]);
  const previewPending = preview.expression !== expression || preview.timeZone !== timeZone;
  const runs = useMemo(() => errors.length || previewPending ? [] : upcomingRuns(fieldsFrom(preview.expression), preview.timeZone), [errors.length, preview, previewPending]);
  const activePreset = presets.find((preset) => cronExpression(preset.value) === expression.trim().replace(/\s+/g, " "))?.label;
  const setValue = (value: CronFields) => setExpression(cronExpression(value));
  ...
  return <div className="cron-editor">
    ...
    <section className="cron-panel cron-editor-box" aria-label="Cron expression editor"><div className="cron-panel-head cron-editor-head"><div className={`cron-expression-bar${errors.length ? " invalid" : ""}`}><input aria-label="Cron expression" aria-describedby="cron-field-labels cron-input-errors" aria-invalid={errors.length > 0} value={expression} onChange={(event) => setExpression(event.target.value)} /><button aria-label="Copy cron expression" onClick={copy}>Copy</button></div><div className="cron-field-labels" id="cron-field-labels">{labels.map((label) => <span key={label}>{label}</span>)}</div><div className="cron-actions"><button onClick={() => setValue(random[Math.floor(Math.random() * random.length)])}>Random example</button><button onClick={() => setExpression("")}>Clear</button><button onClick={() => setValue(initial)}>Reset</button></div></div>{errors.length > 0 && <p className="cron-input-errors" id="cron-input-errors" aria-live="polite">{errors.map((error) => error.message).join(" ")}</p>}<p className="cron-copy-status" aria-live="polite">{copyState}</p></section>
