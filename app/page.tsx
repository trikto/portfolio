...
function DashboardChart({ title, unit, paths, legends }: { title: string; unit: string; paths: Array<[string, string]>; legends: Array<[string, string]> }) {
  return <article className="dashboard-chart"><div className="dashboard-chart-head"><span>{title}</span><small>{unit}</small></div><svg viewBox="0 0 520 150" preserveAspectRatio="none" aria-hidden="true"><path className="chart-grid" d="M0 20H520 M0 60H520 M0 100H520 M0 140H520 M0 0V150 M130 0V150 M260 0V150 M390 0V150 M520 0V150" />{paths.map(([tone, path]) => <path className={`chart-line ${tone}`} d={path} key={tone} />)}</svg><div className="chart-scale"><span>12:00</span><span>14:00</span><span>16:00</span><span>18:00</span><span>now</span></div><div className="chart-legend">{legends.map(([tone, label]) => <span key={label}><i className={tone} />{label}</span>)}</div></article>;
}

function ObservabilityDashboard() {
  const gauges: Array<[string, string, number, string?]> = [["CPU Busy", "2.6%", 3], ["Sys Load", "0.42", 8], ["RAM Used", "48.1%", 48], ["SWAP Used", "4.0%", 4, "amber"], ["Root FS Used", "61.0%", 61, "amber"], ["CPU Cores", "4", 25], ["Uptime", "18d 06h", 44]];
  ...
  return <section className="section observability-section" aria-labelledby="observability-title">
    ...
    <div className="dashboard-shell node-dashboard">
      <div className="dashboard-head"><div><strong>node exporter / full</strong><span>local illustrative node metrics</span></div><div className="dashboard-controls"><span>env&nbsp;&nbsp; portfolio</span><span>job&nbsp;&nbsp; node</span><span>host&nbsp;&nbsp; platform-01</span><span>last 6 hours</span></div></div>
      ...
      <div className="dashboard-events"><span className="eyebrow">NODE EVENTS</span><div><span><i /> release completed <small>02m</small></span><span><i /> health checks passing <small>18m</small></span><span><i /> backup verified <small>42m</small></span><span><i /> recovery notes updated <small>today</small></span></div></div>
    </div>
  </section>;
}
