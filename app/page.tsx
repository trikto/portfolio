...
function DashboardChart({ title, unit, paths, legends }: { title: string; unit: string; paths: Array<[string, string]>; legends: Array<[string, string]> }) {
  const [hidden, setHidden] = useState<string[]>([]);
  const toggle = (tone: string) => setHidden(current => current.includes(tone) ? current.filter(item => item !== tone) : [...current, tone]);
  return <article className="dashboard-chart"><div className="dashboard-chart-head"><span>{title}</span><small>{unit}</small></div><svg viewBox="0 0 520 150" preserveAspectRatio="none" aria-label={`${title} illustrative chart`}><title>{title} local illustrative series</title><path className="chart-grid" d="M0 20H520 M0 60H520 M0 100H520 M0 140H520 M0 0V150 M130 0V150 M260 0V150 M390 0V150 M520 0V150" />{paths.filter(([tone]) => !hidden.includes(tone)).map(([tone, path]) => <path className={`chart-line ${tone}`} d={path} key={tone} />)}</svg><div className="chart-scale"><span>12:00</span><span>14:00</span><span>16:00</span><span>18:00</span><span>now</span></div><div className="chart-legend">{legends.map(([tone, label]) => <button type="button" className={hidden.includes(tone) ? "is-hidden" : ""} onClick={() => toggle(tone)} aria-pressed={!hidden.includes(tone)} key={label}><i className={tone} />{label}</button>)}</div></article>;
}
...
function ObservabilityDashboard() {
  ...
  const statuses = [["Pressure", "nominal"], ["API", "healthy"], ["App", "4 / 4"], ["Database", "ready"], ["Backups", "verified"]];
  return <section className="section observability-section" aria-labelledby="observability-title">
    ...
    <div className="dashboard-shell node-dashboard">
      ...
      <div className="dashboard-chart-grid"><DashboardChart title="CPU Basic" unit="0 - 100%" paths={[["good", "M0 136 C45 131 72 140 105 126 S157 132 194 116 S248 128 280 108 S340 125 380 98 S434 111 470 88 S500 104 520 82"], ["amber", "M0 142 C70 141 105 138 152 141 S220 135 270 138 S344 133 392 136 S462 129 520 132"]]} legends={[["good", "idle"], ["amber", "busy system"]]} /><DashboardChart title="Memory Basic" unit="0 - 16 GiB" paths={[["blue", "M0 96 C80 95 130 98 200 94 S300 99 360 93 S442 96 520 91"], ["amber", "M0 121 C72 118 140 119 210 116 S335 116 410 112 S480 112 520 108"]]} legends={[["blue", "RAM total"], ["amber", "RAM used"]]} /><DashboardChart title="Network Traffic Basic" unit="receive / transmit" paths={[["good", "M0 78 C55 70 88 89 128 63 S187 84 230 54 S302 77 352 66 S424 88 465 52 S500 71 520 45"], ["violet", "M0 88 C62 98 105 79 150 104 S226 79 272 111 S350 83 395 106 S468 89 520 116"]]} legends={[["good", "receive"], ["violet", "transmit"]]} /><DashboardChart title="Disk Space Used Basic" unit="0 - 100%" paths={[["alert", "M0 74 C80 75 130 73 205 74 S300 75 382 72 S460 73 520 71"], ["teal", "M0 112 C76 111 150 113 220 109 S338 111 418 106 S480 108 520 103"]]} legends={[["alert", "root filesystem"], ["teal", "application volume"]]} /></div>
      <div className="dashboard-events"><span className="eyebrow">NODE EVENTS / {job} / {host} / {range}</span><div><span><i /> release completed <small>02m</small></span><span><i /> health checks passing <small>18m</small></span><span><i /> backup verified <small>42m</small></span><span><i /> recovery notes updated <small>today</small></span></div></div>
    </div>
  </section>;
}
